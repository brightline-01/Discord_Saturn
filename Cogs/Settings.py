...
class Verify_View(discord.ui.View):
    ...
    async def Verify(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        Role_ID = Settings.get(Guild_ID, {}).get("Verify", {}).get("Role_ID")
        
        if not Role_ID:
            return await interaction.response.send_message("이 서버에 설정된 인증 역할이 없습니다. 서버 소유자 또는 관리자에게 문의해주세요.", ephemeral=True)
        
        Role = interaction.guild.get_role(Role_ID)
        if not Role:
            return await interaction.response.send_message("설정된 역할을 찾을 수 없습니다. 서버 소유자 또는 관리자에게 문의해주세요.", ephemeral=True)
...
class Settings(commands.Cog):
    ...
    @discord.default_permissions(administrator=True)
    async def Setup_Warning_Threshold(self, ctx,
        ...
        try:
            ...
            if not enabled:
                ...
                Settings[Guild_ID]["Auto_Punish"] = {"Enabled": True, "Count": count, "Action": action, "Duration": duration if action == "타임아웃" else None}
                Action_Desc = f"**{action}**" + (f" ({duration})" if action == "타임아웃" else "")
            ...
            await ctx.respond(embed=Error_Dialog_Embed(f"설정 저장 오류: {e}"), ephemeral=True)
    ...
    @discord.default_permissions(administrator=True)
    async def Setup_Anti_Spam(self, ctx,
        enabled: discord.Option(bool, name="활성화", description="도배 감지 활성화 여부를 선택하세요."),
        ...
        mode: discord.Option(str, name="모드", description="감지할 메시지의 종류를 지정하세요.", choices=["모든 메시지", "동일한 메시지"]),
        ...
        try:
            ...
            Guild_ID = str(ctx.guild.id)
            if not enabled:
                if Guild_ID in Settings and "Anti_Spam" in Settings[Guild_ID]:
                    del Settings[Guild_ID]["Anti_Spam"]
                Message = "도배 감지 설정을 비활성화했습니다."
            else:
                if Guild_ID not in Settings: Settings[Guild_ID] = {}
                Settings[Guild_ID]["Anti_Spam"] = {"Enabled": True, "Count": count, "Seconds": seconds, "Action": action, "Mode": mode, "Duration": duration if action == "타임아웃" else None}
                Mode_Text = "모든" if mode == "모든 메시지" else "동일한 내용의"
                Message = f"**{seconds}초** 이내에 {Mode_Text} 메시지를 **{count}개** 이상 보낼 경우 자동으로 **{action}**하도록 설정했습니다."
            
            Save_Data(self.Settings_Data_Path, Settings)
            await ctx.respond(embed=Success_Dialog_Embed(Message), ephemeral=True)
            Print_Log("Settings", "도배 감지 설정을 변경했습니다.", ctx.guild.name, ctx.author.name, extra=f"상태: {'활성화' if enabled else '비활성화'}")
        except Exception as e:
            await ctx.respond(embed=Error_Dialog_Embed(f"설정 저장 오류: {e}"), ephemeral=True)
    ...
    @discord.default_permissions(administrator=True)
    async def Setup_Anti_Admin(self, ctx,
        enabled: discord.Option(bool, name="활성화", description="권한 부여 감지 활성화 여부를 선택하세요.")):
        ...
        try:
            ...
            Guild_ID = str(ctx.guild.id)
            if Guild_ID not in Settings: Settings[Guild_ID] = {}
            Settings[Guild_ID]["Anti_Admin"] = {"Enabled": enabled}
            
            Save_Data(self.Settings_Data_Path, Settings)
            Status_Text = "활성화" if enabled else "비활성화"
            Message = f"권한 부여 감지를 **{Status_Text}**했습니다."
            await ctx.respond(embed=Success_Dialog_Embed(Message), ephemeral=True)
            Print_Log("Settings", "권한 부여 감지 설정을 변경했습니다.", ctx.guild.name, ctx.author.name, extra=f"상태: {Status_Text}")
        except Exception as e:
            await ctx.respond(embed=Error_Dialog_Embed(f"설정 저장 오류: {e}"), ephemeral=True)
    ...
    @discord.default_permissions(administrator=True)
    async def Setup_Anti_Raid(self, ctx,
        enabled: discord.Option(bool, name="활성화", description="채널 방어 활성화 여부를 선택하세요."),
        ...
        try:
            ...
            if not enabled:
                if Guild_ID in Settings and "Anti_Channel" in Settings[Guild_ID]:
                    del Settings[Guild_ID]["Anti_Channel"]
                Message = "레이드 감지 설정을 비활성화했습니다."
            else:
                if Guild_ID not in Settings: Settings[Guild_ID] = {}
                Settings[Guild_ID]["Anti_Channel"] = {"Enabled": True, "Count": count, "Seconds": seconds}
                Message = f"**{seconds}초** 이내에 채널을 **{count}개** 이상 생성 또는 삭제할 경우 사용자를 자동으로 **차단**하도록 설정했습니다."
            
            Save_Data(self.Settings_Data_Path, Settings)
            Status_Text = "활성화" if enabled else "비활성화"
            await ctx.respond(embed=Success_Dialog_Embed(Message), ephemeral=True)
            Print_Log("Settings", "레이드 감지 설정을 변경했습니다.", ctx.guild.name, ctx.author.name, extra=f"상태: {Status_Text}")
        except Exception as e:
            await ctx.respond(embed=Error_Dialog_Embed(f"설정 저장 오류: {e}"), ephemeral=True)
    ...
    async def Setup_Ticket_System(self, ctx,
        category: discord.Option(discord.CategoryChannel, name="생성", description="티켓이 생성될 카테고리를 선택하세요.", required=True),
        ...
        try:
            ...
            if category: Settings[Guild_ID]["Ticket"]["Category_ID"] = category.id
            if archive: Settings[Guild_ID]["Ticket"]["Archive_Category_ID"] = archive.id
            if role: Settings[Guild_ID]["Ticket"]["Staff_Role_ID"] = role.id
            if log: Settings[Guild_ID]["Ticket"]["Log_Channel_ID"] = log.id
    ...
    async def Reset_Ticket_System(self, ctx):
        ...
        if View.Value is None:
            await Response.edit(embed=Error_Dialog_Embed("시간이 초과되었습니다."), view=None)
        elif View.Value:
            try:
                Settings = Load_Data(self.Settings_Data_Path)
                Guild_ID = str(ctx.guild.id)
                if Guild_ID in Settings and "Ticket" in Settings[Guild_ID]:
                    del Settings[Guild_ID]["Ticket"]
                    Save_Data(self.Settings_Data_Path, Settings)
                    ...
                    await Response.edit(embed=Error_Dialog_Embed("이 서버에 저장된 티켓 시스템 설정이 없습니다."), view=None)
            except Exception as e:
                await Response.edit(embed=Error_Dialog_Embed(f"티켓 시스템 설정을 초기화하는 중 오류가 발생했습니다. ({e})"), view=None)
        else:
            await Response.edit(embed=Success_Dialog_Embed("티켓 시스템 설정 초기화를 취소했습니다."), view=None)
    ...
    @discord.default_permissions(administrator=True)
    async def Setup_Verify_System(self, ctx,
        role: discord.Option(discord.Role, name="역할", description="인증 시 부여할 역할을 지정하세요.", required=True),
        ...
        try:
            ...
            if Guild_ID not in Settings_Data: Settings_Data[Guild_ID] = {}
            Settings_Data[Guild_ID]["Verify"] = {"Role_ID": role.id}
            
            Save_Data(self.Settings_Data_Path, Settings_Data)
            
            Default_Desc = f"✅ 인증하기 버튼을 클릭하여 인증하세요."
            embed = discord.Embed(
                title="✅ 멤버 인증",
    ...
    async def Reset_Verify_System(self, ctx):
        ...
        if View.Value is None:
            ...
            try:
                Settings_Data = Load_Data(self.Settings_Data_Path)
                Guild_ID = str(ctx.guild.id)
                if Guild_ID in Settings_Data and "Verify" in Settings_Data[Guild_ID]:
                    del Settings_Data[Guild_ID]["Verify"]
                    Save_Data(self.Settings_Data_Path, Settings_Data)
                    ...
                    await Response.edit(embed=Error_Dialog_Embed("이 서버에 저장된 인증 시스템 설정이 없습니다."), view=None)
            except Exception as e:
                await Response.edit(embed=Error_Dialog_Embed(f"인증 시스템 설정을 초기화하는 중 오류가 발생했습니다. ({e})"), view=None)
        else:
            await Response.edit(embed=Success_Dialog_Embed("인증 시스템 설정 초기화를 취소했습니다."), view=None)
