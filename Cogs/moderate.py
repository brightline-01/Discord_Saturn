...
class moderate(commands.Cog):
    ...
    @Moderate.command(name="차단", description="사용자를 서버에서 차단합니다. 멤버 차단 권한을 요구합니다.", options=[
        discord.Option(str, name="사용자", description="차단할 사용자 (멘션 또는 ID)", required=True),
        discord.Option(str, name="사유", description="차단할 사유 (선택)", required=False)])
    @discord.default_permissions(ban_members=True)
    async def ban(self, ctx, member: str, *, reason: str = "사유 없음"):
        user_id_match = re.search(r'\d+', member)
        if not user_id_match:
            return await ctx.respond(embed=discord.Embed(description=":warning: 사용자를 찾을 수 없습니다. 올바른 사용자 멘션 또는 ID를 입력해주세요."), ephemeral=True)
        user_id = int(user_id_match.group())

        if user_id == self.bot.user.id:
            return await ctx.respond(embed=discord.Embed(description=":warning: 애플리케이션이 스스로를 차단할 수 없습니다."), ephemeral=True)
        try:
            user = await self.bot.fetch_user(user_id)
            await ctx.guild.ban(user, reason=reason)
            embed = discord.Embed(title=f":hammer_pick: {user.name}님을 서버에서 차단했습니다.", color=discord.Color.red())
            embed.add_field(name="사유", value=reason if reason else "사유 없음", inline=True)
            embed.add_field(name="요청자", value=ctx.author.display_name, inline=True)
            embed.set_thumbnail(url=user.avatar.url)
            embed.set_footer(text=f"일시: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            await ctx.respond(embed=embed)
            print(f"[Command | Moderate] 사용자를 차단했습니다. (서버: {ctx.guild.name}, 요청자: {ctx.author.name}, 차단한 사용자: {user.name}, 일시: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
        except discord.Forbidden:
    ...
    async def ban_error(self, ctx, error):
        if isinstance(error, commands.BadArgument):
            await ctx.respond(embed=discord.Embed(description=":warning: 사용자를 찾을 수 없습니다. 올바른 사용자 멘션 또는 ID를 입력해주세요."), ephemeral=True)
        elif isinstance(error, commands.MissingPermissions):
